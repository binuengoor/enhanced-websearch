...
| MP-09 Optional product enhancements | done | complete | pass | implemented optional local-first polish: automatic Markdown/YAML research report export to `artifacts/reports`, bounded recent-run history via `/runs/recent`, enhanced canonical `/metrics` surface mirrored by MCP `service_metrics`, compose bind mount for report output, and Dockerfile prep for report directory; no DB, no job system, no duplicate status endpoints |
| MP-10 Research output preservation | in_progress | complete | pass | implementation landed on feature branch; `/research` now returns canonical `body`, preserves substantive Vane longform output, adds deterministic Vane rejection heuristics plus fallback provenance, and extends progress states for vane/vetting/fallback; targeted regression tests passing |
